...
# ================= CONFIG =================
TOKEN = "8149094286:AAGOQKxAHB2mPxDl9_ZjoIMB-ggVQH1OqmI"
...
def handle_file(msg):
    ...
    kb.add(
        ...
        types.InlineKeyboardButton("▶️ STRIP CHARGE 5$", callback_data=f"run|gate_stcharge|{combo}")
    )
...
[+] Dev : {adus}"""
                    ...
                    with open(LOG_FILE,"a") as f:
                        f.write(f"{datetime.now()} | strip card error | {cc} | {e_card}\n")
                    continue
    except Exception as e:
        with open(LOG_FILE,"a") as f:
            f.write(f"{datetime.now()} | strip gate error | {e}\n")
    finally:
            ...
            for cc in cards:
                try:
                    ...
                    if user["points"] < POINTS_PER_CARD:
                        ...
                        return
                    cc = cc.strip()
                    last = run_with_retry(Strip_charge5, cc)
                    remove_points(user_id, POINTS_PER_CARD)
                    user = get_user(user_id)
                    mes = types.InlineKeyboardMarkup(row_width=1)
                    mes.add(
...
"""
                    ...
                    elif "give-form-hash not found" in last or "form-id not found" in last: err += 1
                    else: charge += 1; bot.send_message(user_id,chmsg)
                except Exception as e_card:
                    with open(LOG_FILE,"a") as f:
                        f.write(f"{datetime.now()} | strip charge card error | {cc} | {e_card}\n")
                    continue
    except Exception as e:
        with open(LOG_FILE,"a") as f:
            f.write(f"{datetime.now()} | strip charge gate error | {e}\n")
    finally:
        safe_edit(call.message.chat.id, call.message.message_id, "✅ FINISHED")
...
"""
                    ...
                    with open(LOG_FILE,"a") as f:
                        f.write(f"{datetime.now()} | ppc normal card error | {cc} | {e_card}\n")
                    continue
    except Exception as e:
        with open(LOG_FILE,"a") as f:
            f.write(f"{datetime.now()} | ppc normal gate error | {e}\n")
    finally:
        safe_edit(call.message.chat.id, call.message.message_id, "✅ FINISHED")
...
"""
                    ...
                    with open(LOG_FILE,"a") as f:
                        f.write(f"{datetime.now()} | ppc full card error | {cc} | {e_card}\n")
                    continue
    except Exception as e:
        with open(LOG_FILE,"a") as f:
            f.write(f"{datetime.now()} | ppc full gate error | {e}\n")
    finally:
        safe_edit(call.message.chat.id, call.message.message_id, "✅ FINISHED")
